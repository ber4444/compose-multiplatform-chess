...
- **Game Summary (on-device):** End-of-game counterpart to the coach. `GameScreen`'s game-over popup shows a **Get Coach Summary** button whenever `LocalGameSummaryManager` is non-null; pressing it feeds `GameActions.toPgn(...)` to `GameSummaryManager.triggerSummary(pgn)` → `DefaultGameSummaryOrchestrator` (45 s `withTimeoutOrNull` for the whole PGN-sized generation). Reuses `moveCoachOffline` and the *same* `OnDeviceTextGeneratorFactory` instance the coach warmed up, so there's no second model load. Two intentional differences from the coach: **no response validator** (any non-blank text is accepted — there are no per-move tags to ground against), and it's **pull-based** (nothing runs until the button is pressed). Attached alongside the coach at every entry point, so where the coach is gated off the orchestrator is `null` and the button is a no-op. `GameSummaryEvent.Streaming` exists in the model and is rendered by the UI, but `DefaultGameSummaryOrchestrator` currently only emits `Complete`.
- **Rules Q&A (on-device, retrieval):** `RulesQaScreen` + `RulesQaStateHolder` over `DefaultRulesQaOrchestrator`. Corpus is the bundled `onDeviceAi/src/commonMain/resources/rulesCorpus/passages.tsv` (30 passages + header) looked up by BM25 (`BundledRuleLookupTool`). The answerer is an `expect fun defaultRulesQaAnswerer(lookupTool)`: **iOS** = native Foundation Models `Tool` conformance with `NLEmbedding` ranking, **Android** = `StructuredOutputRulesQaAnswerer` (model emits `{"tool":"lookup_rule","query":"…"}`, Kotlin performs the lookup, a second turn cites the passage), **desktop / wasm / JS return `null`** — the screen reports unavailable there. An answer that cites no retrieved passage ID is rejected in favour of a static rules summary.
