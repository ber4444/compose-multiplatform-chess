...
- **3D Board View:** The `GameScreen` settings row shows a 3D toggle (`viewState.show3D`), but only when a `Board3DSupport` is injected (desktop, iOS, and Android entry points pass their respective `Board3DSupport`; other platforms pass `null` until their backend milestones, hiding the toggle). Toggling **swaps** the board: exactly one board (2D `chess_board` or 3D `board_3d`) is visible at a time. The 3D view is defined by the `Chess3DBoardRenderer` interface in `commonMain`, injected mirroring `ChessEngine`. `Board3DSceneMapper.fromFen` turns a FEN into a renderer-agnostic `Board3DScene`. Interaction (camera drag, ray-picked tap-to-move) uses a shared `OrbitCameraController` and `CameraMath.rayFromScreen` in commonMain, mapped cleanly to backend cameras by strictly syncing FOV and aspect ratios.
  ...
  - **Android backend:** A Filament renderer via **SceneView** (`io.github.sceneview:sceneview`), the Jetpack-Compose-native Filament wrapper (`AndroidSceneViewChessRenderer`). The renderer is a Compose-observable state holder behind `Chess3DBoardRenderer`; `AndroidBoard3DSurface` hosts SceneView's `Scene { }` with `SurfaceType.SurfaceView` (so promotion/game-over/draw dialogs layer above it) and builds board/piece `ModelNode`s from `Board3DSceneMapper`. SceneView's model loader loads the `chess.glb` asset. This replaced the original hand-written Filament `Engine`/`Renderer`/`SwapChain` + `SurfaceHolder.Callback` plumbing; Materia and a raw NDK Vulkan renderer were evaluated and rejected (see `docs/plans/issue-32-3d-ui-m3-android.md` and `docs/plans/issue-32-3d-ui-unresolved-questions.md`).
