...
- **3D Board View:** The `GameScreen` settings row shows a 3D toggle (`viewState.show3D`), but only when a `Board3DSupport` is injected (desktop entry point passes `desktopBoard3DSupport()`; other platforms pass `null` until their backend milestones, so the toggle is hidden there). Toggling **swaps** the board: enabling 3D hides the 2D `Board` (`chess_board` is not composed) and shows the 3D surface (`board_3d`) in its place; exactly one board is visible at a time. Since 3D tap-to-move isn't implemented yet, 3D mode is **view-only** in M1; toggle back to 2D to make moves (engine/autoplay still progress). The 3D view is defined by the `Chess3DBoardRenderer` interface in `commonMain` (`app/src/commonMain/kotlin/com/example/myapplication/board3d/`), injected as a `Board3DSupport` (renderer factory + a platform `surfaceContent` composable), mirroring how `ChessEngine` is injected. `Board3DSceneMapper.fromFen` turns a FEN into a renderer-agnostic `Board3DScene` (pure, unit-tested); `BoardGeometry`/`Math3D` hold the world-space + ray-pick math. If the factory returns `null` (no backend / init failure) the toggle reverts to 2D and shows a `board_3d_unavailable` message. The Desktop backend is an LWJGL **headless Vulkan** renderer (`VulkanChessRenderer`) that reads frames back into a Compose `ImageBitmap`; the offscreen Vulkan pipeline itself is still scaffolding (placeholder frames) pending M1 Phase D, with `jgltf-model` parsing `chess.glb` and `ChessSetMeshNames` mapping pieces to glTF nodes. The Materia engine was evaluated and rejected for desktop (no offscreen path) — see `docs/plans/issue-32-3d-ui-m1-foundation.md`. Run the 3D smoke test with the `chess3d.smoke` system property (see Commands).
