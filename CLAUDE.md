...
- **Rules Q&A (on-device, retrieval):** `RulesQaScreen` + `RulesQaStateHolder` over `DefaultRulesQaOrchestrator`. Corpus is the bundled `onDeviceAi/src/commonMain/resources/rulesCorpus/passages.tsv` (30 passages + header) looked up by BM25 (`BundledRuleLookupTool`). The answerer is an `expect fun defaultRulesQaAnswerer(lookupTool)`: **iOS** = native Foundation Models `Tool` conformance with `NLEmbedding` ranking (same iOS 26+/Apple Intelligence gate as the coach, no separate flag), **Android** = `OnDeviceRulesQaAnswerer` (Kotlin looks up the user's question, the model emits a tool call (native if `supportsTools` is true, otherwise structured JSON output `{"tool":"lookup_rule","query":"…"}`) to *refine* that lookup, and a second turn phrases the result) — since Cactus is initialized at launch on all Android builds (`attachMoveCoach`), `isCactusInitialized()` is true on release, making Rules Q&A active on release Android as well. **desktop / wasm / JS return `null`** unconditionally.
