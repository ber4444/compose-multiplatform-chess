...
- **3D Board View:** The `GameScreen` settings row shows a 3D toggle (`viewState.show3D`), but only when a `Board3DSupport` is injected (desktop, web, iOS, and Android entry points pass their respective `Board3DSupport`; unsupported platforms pass `null`, hiding the toggle). Toggling **swaps** the board: exactly one board (2D `chess_board` or 3D `board_3d`) is visible at a time. The 3D view is defined by the `Chess3DBoardRenderer` interface in `commonMain`, injected mirroring `ChessEngine`. `Board3DSceneMapper.fromFen` turns a FEN into a renderer-agnostic `Board3DScene`. Interaction (camera drag, pinch zoom, ray-picked tap-to-move, and piece transitions) uses shared commonMain logic (`OrbitCameraController`, `CameraMath.rayFromScreen`, `BoardRayPicker`, `Board3DSceneDiffer`) so backend cameras stay in sync with the picker.
  ...
  - **Web backend (current):** **WebGPU + WGSL** via `WebGpuChessRenderer`, which mirrors the desktop WGSL pipeline into an absolutely positioned overlay `<canvas>`. Reuses `WgpuShaders`, the papermill environment cubemap, and `ChessSceneGeometry` from the `wgpuMain` source set.
  - **Web backend (chosen successor):** **three.js** (WebGLRenderer + MeshStandardMaterial + ACESFilmicToneMapping + PCFSoftShadowMap). The Phase B spike (`web/spikes/threeBaseline.js`) loaded `chess.glb` with three's `GLTFLoader` and matched/exceeded Android's per-frame quality at the canonical baseline scenes; see `docs/plans/web-graphics-spike-result.md` (verdict: ADOPTED) and the migration plan in `docs/plans/web-threejs-migration.md`. Production web still ships WebGPU; the migration is tracked as a separate follow-up.
  - **iOS backend (current):** A native SceneKit renderer (`IosSceneKitChessRenderer`). It wraps an `SCNView` inside a `UIKitView` with `interactive = false`, allowing Compose `pointerInput` to intercept touches and seamlessly reuse the common raycast math. It loads geometries via OBJ files exported seamlessly from the Java/Desktop models. (Materia/MoltenVK were evaluated and rejected; see `docs/plans/issue-32-3d-ui-m2-apple.md`.)
  - **iOS backend (chosen successor):** **three.js via WKWebView** (WebGLRenderer + MeshStandardMaterial + ACESFilmicToneMapping + PCFSoftShadowMap). The Phase C spike bundled the three.js chess renderer (`bundle.js` = three.js r169 + OrbitControls + GLTFLoader + RoomEnvironment + the chess scene code, all bundled via esbuild) alongside `chess.glb` and `baseline.html` as iOS app resources, loaded into a WKWebView via `loadHTMLString(_:baseURL:)`. The spike matched/exceeded Android's per-frame quality at the canonical baseline scenes (see `build/ios-threejs-baseline.png`). RealityKit was evaluated and rejected — it cannot load `.glb` natively (ModelIO only handles `.gltf`/`.obj`/`.usdz`), its `USDZExporter` conversion pipeline loses materials, and even a native Sketchfab `.usdz` with embedded materials rendered as a flat gray rectangle due to RealityKit's limited `UsdPreviewSurface` material support. The WKWebView path renders identically to the web target's three.js spike (same bundle, same `chess.glb`, same PBR pipeline). Production iOS still ships SceneKit; the migration is tracked as a separate follow-up. Activate the dev-mode spike via `CHESS_BASELINE_DEMO=1` (see `iosApp/iosApp/ContentView.swift`).
