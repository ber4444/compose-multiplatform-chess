...
- **3D Board View:** The `GameScreen` settings row shows a 3D toggle (`viewState.show3D`), but only when a `Board3DSupport` is injected (desktop entry point passes `desktopBoard3DSupport()`; other platforms pass `null` until their backend milestones, so the toggle is hidden there). Toggling **swaps** the board: enabling 3D hides the 2D `Board` (`chess_board` is not composed) and shows the 3D surface (`board_3d`) in its place; exactly one board is visible at a time. On **desktop**, 3D tap-to-move is wired (tap → ray-pick → the same `updateSelected`/`playerMove` path as the 2D board, with a green selection-square highlight), so 3D mode is playable there; other platforms remain view-only until their backends land. The 3D view is defined by the `Chess3DBoardRenderer` interface in `commonMain` (`app/src/commonMain/kotlin/com/example/myapplication/board3d/`), injected as a `Board3DSupport` (renderer factory + a platform `surfaceContent` composable), mirroring how `ChessEngine` is injected. `Board3DSceneMapper.fromFen` turns a FEN into a renderer-agnostic `Board3DScene` (pure, unit-tested); `BoardGeometry`/`Math3D` hold the world-space + ray-pick math; `setSelectedSquare` carries the (visual-only) highlight. If the factory returns `null` (no backend / init failure) the toggle reverts to 2D and shows a `board_3d_unavailable` message. The **Desktop backend is a working LWJGL headless Vulkan renderer** (`VulkanChessRenderer`, MoltenVK on macOS): it bakes the whole scene into one world-space, CPU-vertex-lit triangle buffer ([ChessSceneGeometry]) and draws it with a single push-constant `viewProj` (no swapchain, no descriptor sets), rendering into an offscreen `VkImage` and copying it back into a Compose `ImageBitmap`. `GltfChessMeshes` parses `chess.glb` via `jgltf-model` (using `ChessSetMeshNames` node names), normalizing the six piece meshes; matrices use JOML. The Materia engine was evaluated and rejected for desktop (no offscreen path) — see `docs/plans/issue-32-3d-ui-m1-foundation.md`. Run/inspect the renderer via the 3D smoke test with the `chess3d.smoke` system property (writes PNGs to `app/build/`; see Commands).
