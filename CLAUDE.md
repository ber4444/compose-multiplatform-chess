...
>
> The 3D actual implementations are finely tuned for each platform's rendering graphics APIs (Vulkan, WebGL-backed Filament, native Filament). Agents will often try to "helpfully" rewrite them to match generic KMP patterns or other platforms' code, which will break the build or rendering pipeline. **Treat these actual implementations as frozen.**

Key `expect/actual` boundaries and platform glue:
- **`Chess3DBoardRenderer` / `Board3DSupport` (3D Renderers)**: Each target has a dedicated implementation relying on platform-specific C/C++/Swift/JS interop. Vulkan (LWJGL) is used for desktop; Filament is used on Android, iOS, and Web/Wasm (Metal on iOS, WebGL through Wasm on web). DO NOT attempt to unify these rendering paths or rewrite the actuals.
