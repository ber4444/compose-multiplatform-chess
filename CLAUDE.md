...
- **Cross-game habit aggregation (B6 / RAG-5):** `HabitAggregator` walks the player's assessed mistakes and blunders (`MoveClass.MISTAKE`/`BLUNDER`) across recent games in `GameHistoryRepository.games`, grouping them by recurring costly motifs (`hangs-piece`) with a general lost-winning-chances rate fallback when no specific motif recurs enough (requiring repetition across multiple games, not multiple blunders in a single bad game). Reprojects existing `MoveAssessment` data into practice positions (`fenBefore` + the engine's `bestMoveSan` alternative) and uses `HabitNarrator` for deterministic headlines and explanations (no LLM, no network). Owned by `HabitsManager` (which observes `GameHistoryRepository.games` and requires no platform runtime or model), displayed in `HabitsScreen` (`Screen.HABITS`), Pro-gated in `AppRoot`, and dynamically advertised in `PaywallScreen`. With Move Coach running deterministically on both phone platforms, Habits serves as the cross-game Pro coaching surface across all platforms.
