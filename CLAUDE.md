...
Compose Multiplatform chess app (Kotlin 2.4.x, Compose Multiplatform 1.10.x) targeting Android, iOS, Linux desktop, macOS desktop, and Web (Wasm). The player picks a side in Settings (`GameViewModel.playerSide`, default White); the other side — `engineSide` — is played by Stockfish where available, otherwise by a simple built-in CPU algorithm. Beyond the game there are two other large subsystems: the 3D board (per-platform Filament backends) and five AI coaching surfaces (three on-device, two cloud-routed through the `:server` module) — see [AI features](#ai-features).
...
- Wasm klib incremental compilation is intentionally disabled in `app/build.gradle.kts`. It was Kotlin 2.3.x that crashed the klib export-name checker on incremental wasm recompiles; the toggle survived the 2.4.x bumps **unverified** — nobody has re-tested whether 2.4.20 still needs it, and the failure only reproduces on a *second, incremental* `:app:compileKotlinWasmJs`, which CI never performs. Re-test before deleting it, and don't read its presence as evidence the bug is still live.
- **The new experimental stdlib predicates (`allDistinct`/`allDistinctBy`/`allEqual`/`allEqualBy`, Kotlin 2.4.20) are used in test sources only.** Not a style preference: `:chess-core`, `:onDeviceAi` and `:coachApi` are published to GitHub Packages and the React Native repo compiles against them, so an experimental stdlib call in *main* code becomes a link error (JS/Native) or `NoSuchMethodError` (JVM) for any consumer on an older stdlib. `@OptIn(ExperimentalStdlibApi::class)` covers the source that uses them; it does not cover the consumer's runtime. There is a genuine call site in `chess-core`'s `DrawConditions.kt` (`nonKings.map { … }.distinct().size == 1` is exactly `allEqual()`) that is deliberately left alone for this reason.
- `gradle.properties` carries two Kotlin 2.4.20 opt-ins: `kotlin.incremental.native=true` (Beta klib IC, local Apple-target builds only) and `kotlin.wasm.compilationMode=multimodule-closed-world-only-in-dev` (per-module wasm binaries for dev builds; production distribution stays `monolith` so the shipped bundle keeps whole-program DCE). Both are one-line reverts.
