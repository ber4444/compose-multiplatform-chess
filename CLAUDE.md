...
Compose Multiplatform chess app (Kotlin 2.3.x, Compose Multiplatform 1.10.x) targeting Android, iOS, Linux desktop, macOS desktop, and Web (Wasm). The player picks a side in Settings (`GameViewModel.playerSide`, default White); the other side — `engineSide` — is played by Stockfish where available, otherwise by a simple built-in CPU algorithm. Beyond the game there are other large subsystems: the 3D board (per-platform Filament backends), cross-game habit aggregation, and five AI coaching surfaces (three on-device, two cloud-routed through the `:server` module) — see [AI features](#ai-features).
...
- `:server` — JVM-only Ktor cloud-AI service: `POST /v1/openings/explain` (one-shot) and `POST /v1/positions/chat/stream` (SSE), both over one in-process corpus with ONNX MiniLM embeddings. **There is no database in the request path.** Deterministic template composers by default; the provider-LLM composers are opt-in via `COACH_LLM_*` env vars and always validated + fallback-guarded. Deployed to Fly.io (see README). **`server/openapi.yaml` is hand-written on purpose** — a spec generated from the routing tree cannot detect server drift, because it *is* the server. `OpenApiContractTest` validates real responses against it; keep the spec independent of the code it checks. **Retrieval is book-first, not embedding-first** — see [Cloud retrieval](#cloud-retrieval).
  ...
  - **Three spend rails, and only the third bounds the bill.** `ProviderCostBudget` caps one
    request (`COACH_LLM_MAX_USD_CENTS`, 1.5c) and `FixedWindowRateLimiter` caps one caller
    (30/min per IP); multiplied, that is ~$27/hour from a single IP on an unauthenticated endpoint,
    with no cap at all on the total across IPs. `SpendLedger` (`SpendLedger.kt`) is the aggregate
    rail: `COACH_LLM_MAX_USD_CENTS_PER_DAY` (default 250c), reserved before each provider call at
    the *same* expected cost the per-request gate prices (`ProviderCostBudget.expectedUsdCents` —
    one definition, so the two cannot drift) and settled afterwards against reported usage. Three
    things are load-bearing: **`main` builds exactly one ledger** and passes it to both
    `defaultDependencies` and `defaultChatDependencies` — the same one-instance rule as
    `RetrievalRuntime`, for the same reason (two half-caps over one provider bill is not the cap the
    operator configured); the refusal is **its own** `ComposeAttempt.LedgerExhausted` and
    `finishReason = "budget_exhausted"`, never folded into `BudgetRejected`, because a mis-sized
    price and an active abuse incident are different incidents; and refusals are logged **first and
    every hundredth** (`SpendLedger.isLoggableRefusal`), since the traffic that exhausts a daily cap
    would otherwise make the log the outage. `0` is a valid cap and means spend nothing — a kill
    switch that needs no redeploy or key rotation. In-process and non-durable by choice: the machine
    scales to zero, and a durable ledger would need the database this service no longer has.
    Attestation (Play Integrity / App Attest / Turnstile) is the *other* half of this problem and is
    still not implemented — it could cover three client targets, but the desktop JVM build has no
    attestation primitive and ships the base URL inside its own jar.
...
- **`GameHistoryRepository`** (`commonMain/.../persistence/GameHistory.kt`) — the list of finished games (`SavedGame` DTOs: id/result/players/moveCount/pgn/moveRecords/playerSide), persisted as one JSON blob under `game_history.v1`, exposed as a `StateFlow<List<SavedGame>>` (newest first, capped at 200). `GameActions` builds the PGN (`PgnSerializer`/`PgnTags`) + `SavedGame` from a `GameUiState` at game end, threading `viewModel.playerSide` so player colors and `SavedGame.playerSide` (`"WHITE"`/`"BLACK"`, default `"WHITE"`) match when playing as Black. `SavedGame.playerSide` is used by `GameHistoryBackfiller` (which only backfills the player's moves) and `HabitAggregator`.
...
- It `remember`s the cloud/on-device holders it owns — `OpeningExplainerStateHolder(createOpeningExplainer())`, `ChatViewModel(createPositionChat())`, and `RulesQaStateHolder(...)` over `defaultRulesQaAnswerer(createBundledRuleLookupTool())` — and closes the first two in `DisposableEffect`. It also `remember`s `HabitsManager(gameHistory)` (when `gameHistory != null`, disposed in `DisposableEffect`) and publishes it via `LocalHabitsManager`. Unlike the AI managers, `HabitsManager` needs no platform runtime, model, or engine — it is a pure aggregation over `gameHistory`. The move-coach and game-summary managers are *not* created here: entry points own them (they need the platform runtime + lifecycle) and pass them in, then `AppRoot` publishes them via `LocalMoveCoachManager` / `LocalGameSummaryManager` / `LocalOpeningExplainerStateHolder` / `LocalHabitsManager` so screens can read them.
...
- **Gating is live, and the `available` flag is the part that's easy to get wrong.** `ProGate`
  ...
  branches on `isProUnlocked()` for the Rules, Chat, and Habits screens (they own their own
  ...
  - `ProGate(available = false)` renders **nothing — not even the upsell**. Selling a feature that
    ...
    return `null` unconditionally). `Screen.HABITS` applies the same logic in `AppRoot`: when
    `habitsManager == null` (no `gameHistory` at this entry point), it renders an informative message
    rather than an upsell. Availability is a property of the **build**, not the entitlement —
    `LocalEntitlements` cannot know it, so it must stay an argument.
  ...
  - **The feature list obeys the same `available` rule as `ProGate`, line by line.**
    `PaywallScreen.proFeatures()` takes each bullet from the signal that gates its surface —
    `MoveCoachManager.hasOrchestrator`, `GameSummaryManager.uiState !is Unavailable`,
    `LocalHabitsManager.current != null` (Habits), `cloudCoachConfigured` (Position Chat and Opening
    Explainer), and a `rulesQaAvailable` argument `AppRoot` fills from its own
    ...
    Game Summary the game-over popup then hid. Gating Habits behind `LocalHabitsManager.current != null`
    prevents selling Habits on builds without game history. When **every** line drops out, the
    purchase button goes with it (Restore stays — it moves no money, and an existing subscriber may
    be on that build), which is why exercising the desktop/wasm purchase layout needs `coach.baseUrl`
    or `CHESS_ENABLE_COACH=1`.
...
- **Cross-game habit aggregation (B6 / RAG-5):** `HabitAggregator` surfaces both tactical habits (recurring costly motifs like `hangs-piece`, or general lost winning chances fallback) and behavioral habits (excessive hint-taking: $\ge 2$ hints in a game, across $\ge 2$ games in the window) alongside each other as part of a unified "player profile" in `HabitsScreen` (`Screen.HABITS`), styled with category badges (`TACTICAL HABIT` / `BEHAVIORAL HABIT`). Hint usage is recorded per-move on `MoveRecord.hintUsed` in `:chess-core`. Reprojects existing `MoveAssessment` and hint data into practice positions (`fenBefore` + `bestMoveSan` or the played hint move) and uses `HabitNarrator` for deterministic headlines and explanations (no LLM, no network). Owned by `HabitsManager` (which observes `GameHistoryRepository.games` and requires no platform runtime or model), Pro-gated in `AppRoot`, and dynamically advertised in `PaywallScreen`. With Move Coach running deterministically on both phone platforms, Habits serves as the cross-game Pro coaching surface across all platforms.
- **Game lifecycle & persistence (issue #39):**
  ...
  - **Game history + save/share** — at game end the popup offers **Save game** (writes a `SavedGame` to `GameHistoryRepository`, double-save guarded) and **Share PGN** (platform `PgnSharer`); the **History** screen lists saved games with a detail view + delete. `SavedGame` records `playerSide` (`"WHITE"`/`"BLACK"`), passed from `viewModel.playerSide` via `GameActions.toSavedGame` and `GameActions.pgnTags`/`toPgn` so player colors and saved games match when playing as Black.
...
- `docs/plans/on-device-coach-rag-unification.md` — **partially implemented**; check before assuming either way. Landed: **RAG-1** (per-ply `MoveAssessment` + motifs, persisted on `MoveRecord`, coach subject switched to the player's moves), **RAG-2** (summary ranks turning points by `cpLoss`/`MoveClass` and cites `[move-N]`), **RAG-3** (the per-move line is grounded in the assessment and the headline is computed in code by `DeterministicCoach`), and the Hint-button split-out from **RAG-4**. Landed since: the **counterfactual** half of RAG-4 — `MoveAssessment.bestMoveSan` (resolved by `SanConverter.sanForUci` at analysis time, where the pre-move position is in scope) drives `DeterministicCoach`'s "Nf3 was stronger." tail and reaches the model as `MoveCoachRequest.betterMoveDisplay`. Silent for BEST/BOOK: inside 10cp the gap is engine noise. **Landed in #136: RAG-5 (cross-game habit aggregation)** — `HabitAggregator` walks the player's assessed MISTAKE/BLUNDER moves in `GameHistoryRepository.games`, groups by recurring costly motifs (e.g. `hangs-piece`) across multiple games with a general winning-chances fallback; `HabitNarrator` deterministically phrases headline and explanation; reprojects `fenBefore` + `bestMoveSan` as practice positions; `HabitsManager` + `HabitsScreen` wired into `AppRoot` as `Screen.HABITS`, Pro-gated and listed on the paywall. Requires no model or network. Not implemented: the rest of RAG-4 (assessment-record retrieval in chat, deterministic-feature query construction) and **RAG-6** (offline chat). Don't document the unimplemented phases as existing behaviour.
