...
class DesktopRendererSmokeTest {
    ...
    private fun assertRealRender(image: ImageBitmap, size: Int) {
        assertTrue(image.width == size && image.height == size)
        val pixels = image.toPixelMap()
        val colors = HashSet<Int>()
        var nonBackground = 0
        for (y in 0 until size step 4) for (x in 0 until size step 4) {
            val c = pixels[x, y]
        ...
        println("[chess3d] distinct colours=${colors.size}, non-background samples=$nonBackground")
    ...
    private fun save(image: ImageBitmap, name: String) {
        runCatching {
            val out = File("build/$name"); out.parentFile?.mkdirs()
            ImageIO.write(image.toAwtImage(), "PNG", out)
            println("[chess3d] wrote ${out.absolutePath}")
        }
    }
}
