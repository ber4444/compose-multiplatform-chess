 ...
 * chances (or where a hint was used), kept so the UI can offer it as a practice position.
 *
 ...
 * history or gameplay behavior. [motif] is a [com.example.myapplication.MotifDetector] slug (e.g.
 * `"hangs-piece"`) or `"excessive-hints"` for behavioral patterns, or `null` for the general "you
 * keep giving back winning chances" fallback used when no single tactic recurs often enough to name.
 *
...
data class HabitSummary(
    ...
    val category: HabitCategory = HabitCategory.TACTICAL,
)
