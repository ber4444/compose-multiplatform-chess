 ...
 *
 * Consumption uses `prepareRequest{}.execute{}` with `bodyAsChannel()` so that **cancelling the
...
class KtorStreamingChatClient(
    ...
    override fun stream(request: PositionChatRequest): Flow<ChatStreamEvent> = flow {
        ...
        // flow's collector cancels the execute{} block, which closes the underlying connection — no
        // orphaned streams (the plan's hard cancellation rule).
