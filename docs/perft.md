...
## Architecture diagram: how the module boundary enables the perft rig

The perft rig's testability rests on a module boundary established before the rig was built. The
diagram below shows how PRs #74 (chess-core extraction) and #79 (`api`/`implementation` split +
`internal` visibility) create the structural conditions that make the perft gate, the Stockfish
localizer, and the MCP server possible. The boundary IS the enabler — "pure functions" are a
consequence of it, not the cause.

```mermaid
graph TB
    subgraph chessCore[":chess-core (PR #74 + #79)"]
        direction TB
        PUBLISHED["Published artifact<br>io.github.ber4444:chess-core<br>(GitHub Packages, tag-driven)"]

        subgraph public["Public API surface (PR #79)"]
            APPLY["applyMove(state, pieceIndex, to, promo)<br>applyWinConditions(state)<br>pickMoveCPU / getLegalMovesForPiece"]
            VM["GameViewModel<br>(StateFlow, no I/O)"]
            CONVERTERS["FenConverter · SanConverter<br>UciMoveConverter · PgnSerializer"]
            SINK["GameSnapshotSink interface<br>(persistence seam)"]
        end

        subgraph internal["internal — invisible to consumers (PR #79)"]
            GEN["getAllLegalMoves<br>getCastlingMoves<br>getEnPassantMoves<br>checkCheck · hasLegalMoves"]
            DRAWS["DrawConditions<br>pieceValueCp<br>DRAW_* thresholds"]
        end

        BOUNDARY["Structural rules (build.gradle.kts)<br>NO Compose · NO russhwolf · NO java.lang.Process<br>NO DrawableResource · NO @Immutable"]
    end

    subgraph testRig["Perft verification rig"]
        direction TB
        KERNEL["Perft.kt<br>legalMovesFor → perft → perftDivide<br>(expands promotion fan-out)"]
        ORACLE1["Oracle 1: PerftPositions.kt<br>(canonical counts — DO NOT EDIT)"]
        ORACLE2["Oracle 2: PerftVsStockfishTest<br>(go perft divide diff → perft-divergence.txt)"]
        TIERS["Three tiers<br>commonTest shallow · desktopTest deep · nightly opt-in"]
    end

    subgraph mcpServer[":perft-mcp (adapter, not engine)"]
        TOOLS["run_perft_gate · stockfish_divide · read_divergence"]
    end

    subgraph consumers["Consumers"]
        APP[":app — Compose UI + platform glue"]
        RN["React Native port<br>(js(IR) artifact — same gate)"]
    end

    %% The boundary enables everything
    BOUNDARY -.->|"enables pure functions"| APPLY
    BOUNDARY -.->|"enables headless testing"| testRig
    internal -.-|"invisible to"| consumers

    %% The test rig calls only the public surface
    KERNEL -->|"calls"| APPLY
    KERNEL -->|"validates against"| ORACLE1
    KERNEL -->|"cross-checks via"| ORACLE2
    ORACLE2 -->|"writes"| DIVFILE["build/perft-divergence.txt"]
    TIERS --> KERNEL

    %% MCP shells out, doesn't call the engine
    TOOLS -.->|"shells out to gradle"| testRig
    TOOLS -.->|"reads"| DIVFILE
    TOOLS -.->|"spawns stockfish directly"| ORACLE2

    %% Published artifact consumed
    PUBLISHED --> APP
    PUBLISHED --> RN
    TIERS -.->|"PerftTest runs on js(IR) target"| RN

    %% Styling
    classDef boundary fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000
    classDef publicApi fill:#e3f2fd,stroke:#1565c0,stroke-width:2px,color:#000
    classDef internalApi fill:#fafafa,stroke:#9e9e9e,stroke-dasharray:5 5,color:#555
    classDef test fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px,color:#000
    classDef mcp fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px,color:#000
    classDef consumer fill:#e0f7fa,stroke:#00838f,stroke-width:2px,color:#000

    class BOUNDARY boundary
    class APPLY,VM,CONVERTERS,SINK publicApi
    class GEN,DRAWS internalApi
    class KERNEL,ORACLE1,ORACLE2,TIERS,DIVFILE test
    class TOOLS mcp
    class APP,RN consumer
```
...
1. **The boundary box (orange) is the root enabler.** The structural rules in `build.gradle.kts`
   (no Compose, no platform I/O, no persistence backend) are *why* `applyMove` is pure — not the
   other way around. A top-level function in a module that transitively depends on `StateFlow` +
   autosave + `java.lang.Process` is not actually pure; the module boundary guarantees it.

2. **The perft rig only touches the public API surface (blue).** `Perft.kt`'s `legalMovesFor`
   calls `applyMove` and reads the board state — it never reaches into the `internal` move-generation
   helpers (gray, dashed). This is the PR #79 visibility perimeter at work: the test harness is a
   consumer like any other, restricted to the documented public surface.

3. **The MCP server (purple) is an adapter, not a caller.** It shells out to gradle and stockfish;
   it has no dependency on `:chess-core` at all. The arrows are dashed because no Kotlin code crosses
   that boundary — only subprocess invocations. This keeps the MCP server from ever re-coupling to
   the engine.

The published artifact feeds two consumers with different UI stacks (the Compose app and the
React Native port), and `PerftTest` in commonTest runs on the `js(IR)` target the RN port consumes —
so the same gate protects the engine in both apps.
