...
cleanly). The `qwen3-0.6` model (~350 MB) is downloaded from Hugging Face by
...
| **Cactus + HF-downloaded Qwen** | **low base APK** | **model-source managed** | **yes after first-launch download** | **M3 ships this path.** ~350 MB `qwen3-0.6` fetched by Cactus into `filesDir`; debug APK ~258 MB. |
