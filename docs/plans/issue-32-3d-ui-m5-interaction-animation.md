...
> **Status:** Feature 1 (tap-to-move + selection highlight) is implemented in the shared `Board3D` host
> and exercised by the active backends. Feature 2's common plumbing is also implemented:
> `Board3DSceneDiffer` computes transitions and the host calls `updatePosition(fen, transition)`.
> Backends that override the overload can animate; backends that do not still snap correctly through the
> default method.

> **Priority note:** Feature 1 (tap-to-move) was **not optional polish** for this project. Because the UI
> hides the 2D board in 3D mode (a swap, not a co-display — see overview), tap-to-move is what makes
> 3D mode playable; animation (Feature 2) is the polish.
